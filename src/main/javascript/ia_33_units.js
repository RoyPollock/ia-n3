...
(function() {
    ...
    var getAllSpec = function() {
        if (!this['all_spec_cache']) {
            ...
            if (this.get('cube') == '2') {
                specs.add('Cube 2.0');
            } else if (this.get('cube') == 'R') {
                /* nothing */
            } else if (this.get('cube') != '') {
                specs.add('Cube');
            }
            if (this.get('irr') != '') {
                specs.add('Irregular');
...
//		return res;
    ...
    var addFlagIcons = units.addFlagIcons = function(unit, target) {
        ...
        } else if (unit.get('cube') == 'R') {
            /* nothing */
        } else if (unit.get('cube') != '') {
    ...
    function loadUnitsRaw(factionName, sectorialName, forMercenaryCompany) {
        ...
        if (sectorialName) {
            ...
            if (!mercenary) {
                if (!forMercenaryCompany) {
                    shouldLoad = function(unit) {
                        return unit.army == factionName || unit.itsMerc || (unit.itsHumanMerc && factionName != "Combined Army" && factionName != "Tohaa");
                    };
                } else {
                    shouldLoad = function(unit) {
